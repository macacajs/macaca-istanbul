...
(function (isNode) {
  ...
  const ESPGEN = isNode ? require('escodegen') : escodegen;  //TODO - package as dependency
  ...
  if (!isNode) {
    ...
    for (const cond in preconditions) {
  ...
  const generateTrackerVar = (filename, omitSuffix) => {
    let suffix;
    if (crypto !== null) {
    ...
    for (let i = 0; i < children.length; i += 1) {
      ...
      if (childNode && !childNode.skipWalk) {
        ...
        if (isArray(childNode)) {
          childArray = [];
          for (let j = 0; j < childNode.length; j += 1) {
  ...
  Walker.prototype = {
    ...
    apply: function (node, walkFn, pathElement) {
      ...
      if (this.debug) {
        ...
        seq = this.seq;
        prefix = '';
        for (let i = 0; i < this.level; i += 1) { prefix += '    '; }
  ...
  Instrumenter.prototype = {
    ...
    instrumentASTSync: function (program, filename, originalCode) {
      ...
      const codegenOptions = this.opts.codeGenerationOptions || { format: { compact: !this.opts.noCompact } };
      codegenOptions.comment = this.opts.preserveComments;
      //console.log(JSON.stringify(program, null, 2));

      const generated = ESPGEN.generate(program, codegenOptions);
      const preamble = this.getPreamble(originalCode || '', usingStrict);
      ...
      if (generated.map && generated.code) {
        ...
        for (let i = 0; i < generated.map._mappings._array.length; i += 1) {
    ...
    getPreamble: function (sourceCode, emitUseStrict) {
      const varName = this.opts.coverageVariable || '__coverage__';
      const file = this.coverState.path.replace(/\\/g, '\\\\');
      const tracker = this.currentState.trackerVar;
      const strictLine = emitUseStrict ? '"use strict";' : '';
      ...
      const coverState = this.opts.debug ? JSON.stringify(this.coverState, null, 2) : JSON.stringify(this.coverState);
      const code = [
        "%STRICT%",
    ...
    statementName: function (location, initValue) {
      const ignoring = !!this.currentState.ignoring;
      location.skip = ignoring || undefined;
      initValue = initValue || 0;
      this.currentState.statement += 1;
      const sName = this.currentState.statement;
    ...
    coverStatement: function (node, walker) {
      ...
      if (this.isUseStrictExpression(node)) {
        const grandParent = walker.ancestor(2);
      ...
      if (node.type === SYNTAX.FunctionDeclaration.name) {
        ...
        const sName = this.statementName(node.loc);

        const incrStatementCount = astgen.statement(
        ...
        if (this.opts.execRuntimeExp || true) {
          const _var = 'window';
          const _method = '_runtime';
          const execRuntimeExp = astgen.statement(
            astgen.callExp(
              astgen.dot(astgen.variable(_var), astgen.variable(_method)),
              astgen.stringLiteral(ESPGEN.generate(node))
            )
          );
          this.splice(execRuntimeExp, node, walker);
        }
      }
    },
    ...
    coverExport: function (node, walker) {
      ...
      const sName = this.statementName(node.declaration.loc);
      const incrStatementCount = astgen.statement(
        astgen.postIncrement(
          astgen.subscript(
            astgen.dot(astgen.variable(this.currentState.trackerVar), astgen.variable('s')),
    ...
    branchName: function (type, startLine, pathLocations) {
      ...
      const bName = this.currentState.branch;
    ...
    ifBranchInjector: function (node, walker) {
      const alreadyIgnoring = !!this.currentState.ignoring;
      const hint = this.currentState.currentHint;
      const ignoreThen = !alreadyIgnoring && hint && hint.type === 'if';
      const ignoreElse = !alreadyIgnoring && hint && hint.type === 'else';
      const line = node.loc.start.line;
      const col = node.loc.start.column;
      const makeLoc = function () {
        return {
          line: line,
          column: col,
        };
      };
      const bName = this.branchName('if', walker.startLineForNode(node), [
        { start: makeLoc(), end: makeLoc(), skip: ignoreThen || undefined },
        { start: makeLoc(), end: makeLoc(), skip: ignoreElse || undefined }
      ]);
      const thenBody = node.consequent.body;
    ...
    switchBranchInjector: function (node, walker) {
      ...
      const bName = this.branchName('switch', walker.startLineForNode(node), this.locationsForNodes(cases));
    ...
    conditionalBranchInjector: function (node, walker) {
      const bName = this.branchName('cond-expr', walker.startLineForNode(node), this.locationsForNodes([node.consequent, node.alternate]));
      const ast1 = this.branchIncrementExprAst(bName, 0);
      const ast2 = this.branchIncrementExprAst(bName, 1);
