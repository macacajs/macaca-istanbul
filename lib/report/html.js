...
// TODO pass
const incrementalFilesMap = {};

const getIcremental = (fileKey, line) => {
  let incremental = false;
  const matrix = incrementalFilesMap[fileKey];
  if (!matrix) {
    return incremental;
  }
...
Report.mix(HtmlReport, {
  ...
  writeDetailPage: function (writer, node, fileCoverage) {
    ...
    const structured = code.map((str) => {
      count += 1;
      const incremental = getIcremental(fileCoverage.path, count);
      return {
        line: count,
        covered: null,
  ...
  writeFiles: function(writer, node, dir, collector) {
    ...
    node.children.forEach((child) => {
      if (child.kind === 'dir') {
        this.writeFiles(writer, child, path.resolve(dir, child.relativeName), collector);
      } else {
        ...
        writer.writeFile(childFile, (contentWriter) => {
          this.writeDetailPage(contentWriter, child, collector.fileCoverageFor(child.fullPath()));
        });
      }
    });
  },
  ...
  writeReport: function (collector, sync) {
    ...
    const writer = opts.writer || new FileWriter(sync);
