...
const genIncrementalCoverage = require('./util/gen-incremental-coverage');
/**
...
Collector.prototype = {
  ...
  addIncrementalFilesMap(incrementMap /* , testName */) {
    this.incrementalFilesMap = incrementMap;
  },
  /**
  ...
  fileCoverageFor(fileName) {
    const ret = this.store.getObject(fileName);
    utils.addDerivedInfoForFile(ret);
    return ret;
  },
  incrementFileCoverageFor(fileCoverage, fileName) {
    const incrementData = this.incrementalFilesMap[fileName];
    if (!incrementData) {
      return null;
    }
    return genIncrementalCoverage(fileCoverage, incrementData);
  },
  /**
  ...
  getFinalCoverage() {
    ...
    this.files().forEach(file => {
      ret[file] = this.fileCoverageFor(file);
    });
    return ret;
  },
  /**
